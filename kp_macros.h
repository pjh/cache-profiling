...
#define kp_log(fp, f, a...)  do {			     \
	fprintf(fp, "KP: %lu: " f, pthread_self(), ##a); \
	fflush(stdout); \
	} while(0)
#else
...
#define kp_test(f, a...) fprintf(stdout, "TEST: %lu: " f, pthread_self(), ##a)
...
#define kp_debug2(f, a...)  do { \
	fprintf(stdout, "DEBUG2: %lu: %s: " f, pthread_self(), __func__, ##a); \
	fflush(stdout); \
	} while(0)
#else
...
#define kp_debug_lock(f, a...)  do { \
	fprintf(stdout, "DEBUGL: %lu: %s: " f, pthread_self(), __func__, ##a); \
	fflush(stdout); \
	} while(0)
#else
...
#define kp_debug_lock2(f, a...)  do { \
	fprintf(stdout, "L: %lu: %s: " f, pthread_self(), __func__, ##a); \
	fflush(stdout); \
	} while(0)
#else
